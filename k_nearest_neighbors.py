    ...
    **Implementation**
    ...
    def euclidean_distance(self, a, b):
        ...
        for index in range(len(a)):
            ...
            Calculation: Subtract b from a, square difference, add to eucl_distance.
    ...
    def fit_knn(self, X_train, y_train):
        ...
        NOTE: I specifically choose not to include a "build-in" data split function, as it allows more flexibility in choosing data split methods according preference and/or data problem. 
    ...
    def predict_knn(self, X):
        ...
        for index in range(len(x)): # Main loop iterating through len(X)
            for row in self.X_train:  # for every row in X_train, find eucl_distance to X using euclidean_distance() and append to euclidian_distances list
                eucl_distance = self.euclidean_distance(row, X[index])
                euclidian_distances.append(eucl_distance)
                
            # sort euclidian_distances in ascending order, and retain only k neighbors as specified in n_neighbors (n_neighbors = k)
            neighbors = np.array(euclidian_distances).argsort()[:n_neighbors]
